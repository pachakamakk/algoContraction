...
int main() {
  ...
  for (int i = 0; i < 10000; ++i) graph.AddNode(i);
  for (int i = 0; i < 10000; ++i) {
    const vector<int>& l1 = graph.OutgoingArcs(i);
    const vector<int>& l2 = graph.IncomingArcs(i);
    CHECK_LE(l1.size(), 30);
    CHECK_LE(l2.size(), 30);
  }
  for (int arc = 0; arc < graph.NumArcs(); ++arc) {
    CHECK_LE(graph.Head(arc), 9999);
    CHECK_LE(graph.Tail(arc), 9999);
  }
